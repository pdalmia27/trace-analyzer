...
def build_visual_summary(input_dir: Path) -> str:
    ...
    osl_task_table_rows = []
    for task in focus_task_names:
        task_rows_for_osl = [row for row in osl_rows if row["task_name"] == task]
        if not task_rows_for_osl:
            continue

        def avg_osl_column(column: str) -> float | None:
            values = [v for v in (safe_float(row[column]) for row in task_rows_for_osl) if v is not None]
            return mean(values)

        osl_task_table_rows.append(
            [
                parse_task_name(task),
                len(task_rows_for_osl),
                fmt_num(avg_osl_column("rollout_count")),
                fmt_num(avg_osl_column("osl_mean_tokens")),
                fmt_num(avg_osl_column("osl_p50_tokens")),
                fmt_num(avg_osl_column("osl_p90_tokens")),
                fmt_num(avg_osl_column("osl_p99_tokens")),
                fmt_num(avg_osl_column("osl_max_tokens")),
            ]
        )
